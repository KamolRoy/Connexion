 ...
 * @author kamol
 */
public class ChannelImage extends JFrame {
    ...
    private int len;
    private Vector TRX, TRX_D, TS, TRXTS, currentChannel, channelStatus;
    private Vector TSA[] = new Vector[8];
    private Vector TSB[] = new Vector[8];
    private Vector TSC[] = new Vector[8];
    
    ChannelImage(String command, String BSC_IP, String Server_IP,String SiteName) {
    ...
    JPanel refreshwork(){
        ...
        String S = TelResult(command_S, BSC_IP_S, Server_IP_S);
        String[] S1 = S.split("END");


        for (int j = 0; j < S1.length; j++) {
            try {
                String S2 = S1[j].substring(S1[j].indexOf("CHNSTAT Result"), S1[j].indexOf("Number of results"));
                String[] S3 = S2.split("\n");
                len = S3.length;

                for (int i = 4; i < len - 1; i++) {
                    //System.out.println(S3[i]);
                    //System.out.println(S3[i].substring(78, 85));
                    TRX.addElement(S3[i].substring(0, 2).trim());
                    TS.addElement(S3[i].substring(9, 10).trim());
                    TRXTS.addElement("TRX" + S3[i].substring(0, 2).trim() + "-" + S3[i].substring(9, 10).trim());
                    currentChannel.addElement(S3[i].substring(39, 50).trim());
                    channelStatus.addElement(S3[i].substring(78, 85).trim());
                }
            } catch (Exception e4) {
            }
        }
